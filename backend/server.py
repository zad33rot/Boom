...
def finalize(req: FinalRegReq):
    ...
    hashed_pw = hash_password(req.password) # Хешируем!
    ...
    cur.execute("UPDATE users SET username=?, nickname=?, password=?, avatar_color=?, auth_code=NULL WHERE email=?", 
                (req.username, req.nickname, hashed_pw, avatar, req.email))
...
def login(req: LoginReq):
    ...
    if res and check_password(req.password, res[0]): # Проверяем хеш
...
def get_messages(email: str):
    ...
    for r in cur.fetchall():
        try:
            decrypted_text = decrypt_msg(r[2]) # Расшифровываем для фронтенда
        except:
            decrypted_text = "[Ошибка расшифровки]"
        
        msgs.append({
            "sender": r[0], "receiver": r[1], "text": decrypted_text, 
            "time": r[3][11:16], "read": bool(r[4])
        })
    ...
    try:
        while True:
            ...
            if data.get("type") == "msg":
                ...
                conn = sqlite3.connect("messenger.db"); cur = conn.cursor()
                cur.execute("INSERT INTO messages (sender_email, receiver_email, text, is_read) VALUES (?, ?, ?, 0)", 
                            (email, receiver, encrypted_text))
                conn.commit(); conn.close()
